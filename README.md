...
The two percentages measure different things and are labeled as such: `ctx` is how full this session's context window is; the `5h`/`7d`/`spend`/`glm`-labeled quota is how much of the account's rate-limit/billing window is consumed. Each segment carries a monochrome Nerd Font icon (branch U+E0A0, repo U+F07B, model U+F544, context U+F080, quota U+F252) drawn in the segment's color — these are private-use glyphs, so a Nerd Font / powerline-patched terminal font is required; delete the icons in the `parts` lines for a plainer line.
